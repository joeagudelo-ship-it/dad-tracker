...
export default async function ShiftPage() {
  ...
  const shiftColors: Record<string, string> = {
    Morning: "bg-[#fef3c7] dark:bg-[#422006] text-[#d97706]",
    Afternoon: "bg-[#ffedd5] dark:bg-[#431407] text-[#ea580c]",
    Evening: "bg-[#dbeafe] dark:bg-[#1e3a5f] text-[#2563eb]",
    Night: "bg-[#ede9fe] dark:bg-[#2e1065] text-[#7c3aed]",
  };
    ...
    <div className="space-y-5">
      ...
      {entries.length === 0 ? (
        <EmptyState
          ...
          description="Add who is covering and when"
          icon={emptyIcon}
        />
      ) : (
        <div className="space-y-3">
          {entries.map((row: string[], i: number) => {
            const shiftClass = shiftColors[row[2]] || "bg-[#f1f5f9] dark:bg-[#334155] text-[#64748b]";
            return (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${shiftClass}`}>
                        {row[2] || "Shift"}
                      </span>
                    </div>
